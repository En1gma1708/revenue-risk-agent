...
| **Reflection** (generator produces, evaluator critiques, loop until it passes) | **Real, load-bearing — a genuine checker/reviewer agent, added and live-verified 2026-08-30** | The guardrail check (`propose_intervention` → guardrail → `execute_action`) is NOT Reflection — it's a **deterministic code check against fixed rules**, no second LLM pass involved, and re-labeling it as Reflection would be wrong (the model doesn't know NPCI's attempt count better on a re-read of the same context). The router's retry-on-missing-handoff logic isn't Reflection either — it detects a missing/invalid TOOL CALL and retries the call, not a model critiquing its own completed output. What genuinely IS Reflection: a separate **checker agent** (`CheckerVerdict`/`_make_checker`/`_run_checker_review`, `pydantic_agents.py`) that reviews a specialist's ALREADY-COMPLETED decision — case facts, final action, reasoning, guardrail result — and judges whether it's actually sound, independent of whether it passed guardrails. Two deliberate cost controls make this viable on an already quota-constrained project: (1) single-shot review of the finished artifact, never a live re-run of the specialist's whole multi-turn tool-calling loop — one cheap judge call instead of a second expensive agentic run; (2) a cheap, structural, no-LLM trigger rule (`_needs_checker_review`: any `HARD_STOP` hit, final tier `APPROVE_FIRST`, or router severity `high`) — not "check everything," which would double real cost for little added value on routine cases. On a flag, the checker can recommend `retry_specialist` (bounded to exactly one retry, never re-checked again — no check-retry-check loop by construction) or `escalate_to_human`; every review, sound or flagged, produces its own `DecisionLogEntry` so approvals are as visible in the audit trail as flags. **Verified on real quota, not just mocked**: a deliberately-constructed contradictory decision (reasoning claiming success on an action the guardrail engine had actually HARD_STOP'd) correctly triggered `escalate_to_human`; a deliberately-mismatched decision (reasoning describing facts that didn't match the real case) correctly triggered `retry_specialist`, and the real specialist re-ran and produced a genuinely different, internally-consistent decision on retry — not a blind re-run. |
...
**Net result: 3 patterns are genuinely load-bearing (tool use, orchestrator-worker, Reflection), 1 is
a real supporting detail (memory/state), 1 is deliberately absent (planning — no genuinely ambiguous
goal to decompose here).** Orchestrator-worker's own history is the
